...
---@param args { data: vim.event.lspattach.data }
...
Config.on(
  "TermRequest",
  ---@param ev { data: vim.event.termrequest.data }
  function(ev)
    if ev.data.sequence:find("^\27%][02];") then
      vim.cmd.redrawtabline()
    end
  end,
  {
    desc = "Redraw when OSC0 or OSC2 is issued (set title)",
  }
)
